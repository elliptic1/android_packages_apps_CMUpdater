...
public class SettingsFragment extends PreferenceFragment implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSystemRom = SysUtils
				.getSystemProperty(Customization.SYS_PROP_MOD_VERSION);
	...
	public void onSharedPreferenceChanged(SharedPreferences mPrefs, String key) {
		if (mUpdateCheck == null || mUpdateType == null) {
			Log.d(TAG, "one of them was null in OSPCL ");
		} else {
			Log.d(TAG, "changed was " + key);
			Log.d(TAG, "muc key is " + mUpdateCheck.getKey());
			Log.d(TAG, "mut key is " + mUpdateType.getKey());
			if (key.equals(mUpdateCheck.getKey())) {
				int value = Integer.valueOf(mPrefs.getString(key, ""
						+ Constants.UPDATE_FREQ_AT_BOOT));
				mPrefs.edit().putInt(Constants.UPDATE_CHECK_PREF, value)
						.apply();
				mUpdateCheck.setSummary(mapCheckValue(value));
				scheduleUpdateService(value * 1000);

			} else if (key.equals(mUpdateType.getKey())) {
				int value = Integer.valueOf(mPrefs.getString(key,
